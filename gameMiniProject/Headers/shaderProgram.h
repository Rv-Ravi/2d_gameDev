...
public:
	...
	void setUniValueM(const char* varName, const varType& value, uint16_t size)
	{
		...
		switch (size)
		{
		case 2: ErrCheck(glUniformMatrix2fv(tmpVal,1,GL_FALSE, &value[0].x)); break;
		case 3: ErrCheck(glUniformMatrix3fv(tmpVal, 1, GL_FALSE, &value[0].x)); break;
		case 4: ErrCheck(glUniformMatrix4fv(tmpVal, 1, GL_FALSE, &value[0].x)); break;
		default: printf("No such value is found for setting float* type as uniform."); break;
		}
	}
